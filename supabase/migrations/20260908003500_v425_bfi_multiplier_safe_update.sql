-- KeySuite V4.25 — BFI multiplier safe-update repair.
-- Supabase safe-update protection rejects UPDATE statements without a WHERE clause.
...
begin
  ...
  if v_cur not in ('USD','RMB') then
    raise exception 'BFI multiplier only supports USD or RMB.';
  end if;
  if coalesce(p_multiplier,0)<=0 then
    raise exception 'BFI multiplier must be greater than zero.';
  end if;
  ...
  if v_count<>1 then
    raise exception 'KeySuite App Settings default row was not found.';
  end if;
  return true;
end $$;
