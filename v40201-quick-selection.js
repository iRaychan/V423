/* KeySuite V4.23.07 — Brand-level tick controls all visible series; CHC and BFI Enhanced are available but default unticked. Dashboard Brand / Series follows User Assigned when no customer, and User Assigned × Customer Price Preference when selected.
...
function syncEnhancedChecks(){
  ...
  const valid=new Set(entries().filter(e=>e.family==='CHC'||e.family==='BFI').map(e=>String(e.key)));
...
function presentationContext(e,data){
  ...
  const selling=seriesFor(e.brand,family,group),brandSeries=brandSeriesFor(e.brand,family,group),master=family==='CHC'?'CHC':(norm(base.masterSeries)||family),masterModel=norm(data?.model),isEnhanced=(family==='CHC'||family==='BFI')&&!!data?.enhanced,displayModel=enhancedAliasModel(masterModel,e,isEnhanced);
...
async function openCurve(e,data){
  ...
  const payload={type:'KEYSUITE_DASHBOARD_OPEN_MODEL',requestId:req.requestId,flowM3h:req.flowM3h,headM:req.headM,rawFlow:req.rawFlow,rawHead:req.rawHead,rawFlowText:req.rawFlowText,rawHeadText:req.rawHeadText,flowUnit:req.flowUnit,headUnit:req.headUnit,quickSelection:true,enhanced:(e.family==='CHC'||e.family==='BFI')&&!!data.enhanced,model:wantedModel};
...
window.KeySuiteV39442Dashboard={version:'4.22.03',runSelected,renderResults,renderPreference,loadPreference,masterSeries,seriesFor,brandSeriesFor,cancelPending,removeMaterialControls,presentationContext};
